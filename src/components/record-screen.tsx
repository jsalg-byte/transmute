...
import { acceptFriendRequest, addWorkoutPlanDay, createFood, createMealLog, createWorkoutPlan, deleteWorkoutSession, getRecord, rejectFriendRequest, removeFriend, sendFriendRequest, setActiveWorkoutPlan, signOut, startWorkoutSession, updateFasting, updateWeightUnit, type TransmuteRecord } from '../lib/api';
...
function AreaContent({ area, record: r, refresh }: { area: Area; record: TransmuteRecord; refresh: () => Promise<void> }) {
  ...
  if (area === 'friends') return <FriendsContent record={r} refresh={refresh} />;
  if (area === 'settings') return <SettingsContent record={r} refresh={refresh} />;
...
function FriendsContent({ record, refresh }: { record: TransmuteRecord; refresh: () => Promise<void> }) {
  const [username, setUsername] = useState('');
  const [notice, setNotice] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const run = async (operation: () => Promise<unknown>, success: string) => { setSaving(true); setNotice(null); try { await operation(); await refresh(); setNotice(success); } catch (reason) { setNotice(reason instanceof Error ? reason.message : 'Unable to update friends.'); } finally { setSaving(false); } };
  const incoming = record.friends.incoming.filter((request) => request.status === 'pending');
  const outgoing = record.friends.outgoing.filter((request) => request.status === 'pending');
  const friends = [...record.friends.incoming, ...record.friends.outgoing].filter((request) => request.status === 'accepted');
  return <><Text style={styles.eyebrow}>THE COMPANY</Text><Text style={styles.title}>Friend</Text><Text style={styles.body}>Send requests by username. Accepted friends can see workout sessions only.</Text><View style={styles.formCard}><Text style={styles.cardTitle}>Add friend</Text><TextInput value={username} onChangeText={setUsername} autoCapitalize="none" placeholder="Friend username" placeholderTextColor="#655D57" style={styles.input} onSubmitEditing={() => void run(async () => { const value = username.trim(); if (value.length < 3) throw new Error('Enter a username with at least 3 characters.'); await sendFriendRequest(value); setUsername(''); }, 'Friend request sent.')} returnKeyType="done" /><Pressable disabled={saving} onPress={() => void run(async () => { const value = username.trim(); if (value.length < 3) throw new Error('Enter a username with at least 3 characters.'); await sendFriendRequest(value); setUsername(''); }, 'Friend request sent.')} style={[styles.actionButton, saving && styles.buttonDisabled]}><Text style={styles.actionButtonText}>Send request</Text></Pressable></View>{notice ? <Text style={styles.notice}>{notice}</Text> : null}<Text style={[styles.eyebrow, styles.section]}>INCOMING REQUESTS</Text>{incoming.length ? incoming.map((request) => <View key={request.id} style={styles.card}><Text style={styles.cardTitle}>{request.name ?? request.username}</Text><Text style={styles.cardMeta}>@{request.username}</Text><View style={styles.inlineActions}><Pressable disabled={saving} onPress={() => void run(() => acceptFriendRequest(request.id), 'Friend request accepted.')}><Text style={styles.inlineAction}>Accept</Text></Pressable><Pressable disabled={saving} onPress={() => void run(() => rejectFriendRequest(request.id), 'Friend request declined.')}><Text style={styles.inlineAction}>Decline</Text></Pressable></View></View>) : <Card title="No incoming requests" /> }<Text style={[styles.eyebrow, styles.section]}>FRIENDS</Text>{friends.length ? friends.map((friend) => <View key={friend.id} style={styles.card}><Text style={styles.cardTitle}>{friend.name ?? friend.username}</Text><Text style={styles.cardMeta}>@{friend.username}</Text><Pressable disabled={saving} onPress={() => void run(() => removeFriend(friend.userId), 'Friend removed.')}><Text style={styles.inlineAction}>Remove</Text></Pressable></View>) : <Card title="No friends yet" /> }<Text style={[styles.eyebrow, styles.section]}>SENT REQUESTS</Text>{outgoing.length ? outgoing.map((request) => <Card key={request.id} title={request.name ?? request.username} meta="Pending" />) : <Card title="No pending sent requests" />}</>;
}

function SettingsContent({ record, refresh }: { record: TransmuteRecord; refresh: () => Promise<void> }) {
  const [notice, setNotice] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const chooseUnit = async (weightUnit: 'kg' | 'lbs') => { setSaving(true); setNotice(null); try { await updateWeightUnit(weightUnit); await refresh(); setNotice('Weight unit saved.'); } catch (reason) { setNotice(reason instanceof Error ? reason.message : 'Unable to save preferences.'); } finally { setSaving(false); } };
  return <><Text style={styles.eyebrow}>THE INSTRUMENT</Text><Text style={styles.title}>Your preferences</Text><Text style={styles.body}>Set the terms of the record.</Text><View style={styles.formCard}><Text style={styles.cardTitle}>Weight unit</Text><View style={styles.exercisePicker}>{(['lbs', 'kg'] as const).map((unit) => <Pressable key={unit} disabled={saving} onPress={() => void chooseUnit(unit)} style={[styles.exerciseOption, record.settings.weight_unit === unit && styles.exerciseOptionActive]}><Text style={[styles.exerciseOptionText, record.settings.weight_unit === unit && styles.exerciseOptionTextActive]}>{unit}</Text></Pressable>)}</View><Text style={styles.cardMeta}>{record.settings.active_routine_id ? 'An active workout plan is selected.' : 'No active workout plan selected.'}</Text></View>{notice ? <Text style={styles.notice}>{notice}</Text> : null}</>;
}

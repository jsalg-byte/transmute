...
import { createPaletteProxy, createThemedStyleProxy, useTransmuteTheme } from '../../theme/transmute-theme';
  ...
  return <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
    ...
    <ScrollView contentContainerStyle={styles.scrollContent}>
      <View style={styles.page}>
        ...
        {!detail && !error ? <View style={styles.loading}><ActivityIndicator color={palette.oxide} /><Text style={styles.body}>Loading session…</Text></View> : null}
            ...
            <View style={styles.primaryColumn}>
              ...
              {selectedExercise ? <>
                ...
                <Pressable accessibilityRole="button" accessibilityState={{ expanded: demoExpanded }} accessibilityLabel={`${demoExpanded ? 'Collapse' : 'Expand'} demonstration for ${selectedExercise.name}`} onPress={() => setDemoExpanded((current) => !current)} style={[styles.demoToggle, demoExpanded && styles.demoToggleOpen]}>
                  ...
                  {demoExpanded ? <ChevronUp color={palette.oxide} size={21} strokeWidth={2.4} /> : <ChevronDown color={palette.oxide} size={21} strokeWidth={2.4} />}
                </Pressable>
...
function MovementTabs({ value, sessionOptions, setsByExercise, onChange, onAddMovement }: {
  ...
  return <View style={styles.movementNavigator}>
    <View style={styles.movementHeadingRow}>
      <Pressable accessibilityRole="button" accessibilityLabel="Previous movement" disabled={activeIndex <= 0} onPress={() => selectAdjacent(-1)} hitSlop={8} style={[styles.movementDirection, activeIndex <= 0 && styles.movementDirectionDisabled]}><ChevronLeft color={palette.oxide} size={22} strokeWidth={2.4} /></Pressable>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel="Add a movement" onPress={onAddMovement} style={styles.addMovementAction}><Plus color={palette.oxide} size={16} strokeWidth={2.5} /><Text style={styles.addMovementActionText}>Add movement</Text></Pressable>
  </View>;
}
